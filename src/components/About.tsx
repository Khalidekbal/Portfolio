        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center mb-16">
          ...
          >
            <div className="relative group">
              {/* Outer Glow aura */}
              <div className="absolute -inset-1 rounded-3xl bg-gradient-to-r from-cyan-500 via-teal-500 to-blue-600 opacity-60 blur-xl group-hover:opacity-90 transition duration-500" />

              {/* Main Image Container */}
              <div className="relative rounded-2xl overflow-hidden bg-slate-900 border-2 border-slate-700/80 p-2 shadow-2xl">
                <img
                  src="/images/profile.jpg"
                  alt="Khalid Mohamed"
                  className="w-72 sm:w-80 h-96 object-cover object-top rounded-xl grayscale-[20%] group-hover:grayscale-0 transition-all duration-500"
                  onError={(e) => {
                    // Fallback avatar if local image path fails
                ...
                <div className="absolute bottom-5 left-5 right-5 p-3 rounded-xl bg-slate-950/80 backdrop-blur-md border border-slate-800 text-xs flex items-center justify-between shadow-lg">
                  <div className="flex items-center space-x-2">
                    <div className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse" />
                    <span className="text-slate-200 font-medium">Ain Shams Univ.</span>
                  </div>
                  <span className="text-cyan-400 font-mono text-[11px]">Class of 2027</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 w-full max-w-sm mt-6">
              <div className="text-center p-3 rounded-xl bg-slate-900/60 border border-slate-800">
                <span className="block text-lg sm:text-xl font-bold text-cyan-400 font-mono">1,000+</span>
                <span className="text-[10px] sm:text-xs text-slate-400">Print Hours</span>
              </div>
              <div className="text-center p-3 rounded-xl bg-slate-900/60 border border-slate-800">
                <span className="block text-lg sm:text-xl font-bold text-teal-400 font-mono">Formula</span>
                <span className="text-[10px] sm:text-xs text-slate-400">EV Powertrain</span>
              </div>
              <div className="text-center p-3 rounded-xl bg-slate-900/60 border border-slate-800">
                <span className="block text-lg sm:text-xl font-bold text-blue-400 font-mono">R&D</span>
                <span className="text-[10px] sm:text-xs text-slate-400">El Sewedy</span>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="lg:col-span-7 space-y-5 text-slate-300 text-sm sm:text-base leading-relaxed"
          >
            <div className="p-6 rounded-2xl glass-panel relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-cyan-500/10 rounded-full blur-2xl" />
              <h3 className="text-lg sm:text-xl font-bold text-slate-100 mb-3 flex items-center space-x-2">
              ...
              <p className="mt-3">
                From formulating battery pack safety strategies in the <strong className="text-cyan-300">ASU EV Racing Team</strong> to architecting smart meter telemetry during my R&D internship at <strong className="text-teal-300">El Sewedy Electrometer</strong>, I prioritize reliability, clean modular architectures, and rigorous validation through SIL/HIL simulation and physical prototypes.
              </p>
              <p className="mt-3">
                My biomechatronics research focuses on demystifying human-machine interfaces—capturing muscle action micro-potentials via <strong className="text-emerald-300">surface EMG (sEMG)</strong> and applying digital signal processing to translate biological intent into fluid prosthetic kinematics.
              </p>
            </div>

            {/* Quick checkmarks */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
              <div className="flex items-start space-x-2.5 p-3 rounded-xl bg-slate-900/40 border border-slate-800/60">
                <CheckCircle2 className="w-4 h-4 text-cyan-400 shrink-0 mt-0.5" />
                <span className="text-xs sm:text-sm text-slate-300">Deterministic Real-Time Firmware (FreeRTOS)</span>
              </div>
              <div className="flex items-start space-x-2.5 p-3 rounded-xl bg-slate-900/40 border border-slate-800/60">
                <CheckCircle2 className="w-4 h-4 text-teal-400 shrink-0 mt-0.5" />
                <span className="text-xs sm:text-sm text-slate-300">Micro-ROS Distributed Multi-Agent Control</span>
              </div>
              <div className="flex items-start space-x-2.5 p-3 rounded-xl bg-slate-900/40 border border-slate-800/60">
                <CheckCircle2 className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" />
                <span className="text-xs sm:text-sm text-slate-300">High-Fidelity CAD & Additive Prototyping</span>
              </div>
              <div className="flex items-start space-x-2.5 p-3 rounded-xl bg-slate-900/40 border border-slate-800/60">
                <CheckCircle2 className="w-4 h-4 text-purple-400 shrink-0 mt-0.5" />
                <span className="text-xs sm:text-sm text-slate-300">Digital Signal Processing & Biomechanics</span>
              </div>
            </div>
          </motion.div>
        </div>
            ...
            {pillars.map((pillar, index) => {
              ...
              return (
                <motion.div
                  ...
                  className={`p-6 rounded-2xl glass-panel ${pillar.borderColor} border transition-all duration-300 hover:-translate-y-1 relative group`}
                >
                  <div className={`w-12 h-12 rounded-xl ${pillar.bgColor} flex items-center justify-center mb-5 border border-slate-700/50 group-hover:scale-110 transition-transform`}>
