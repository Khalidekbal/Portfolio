        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {certificatesData.map((cert, index) => (
            <motion.div
              ...
              className="group cursor-pointer rounded-2xl glass-panel border border-slate-800 p-5 flex flex-col justify-between hover:border-cyan-500/40 transition-all duration-300 hover:shadow-xl hover:shadow-cyan-500/10 hover:-translate-y-1 relative"
            >
              {/* Media Thumbnail Container */}
              <div className="relative h-44 w-full rounded-xl overflow-hidden bg-slate-950 border border-slate-800/80 mb-4 flex items-center justify-center group-hover:border-cyan-500/30 transition-colors">
                {cert.mediaType === 'image' ? (
                  ...
                  <div className="flex flex-col items-center justify-center space-y-2 p-4 text-center">
                    ...
                    <span className="text-xs font-mono text-cyan-300">PDF Certificate Document</span>
                  </div>
                )}

                {/* Hover overlay preview badge */}
                <div className="absolute inset-0 bg-slate-950/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <span className="flex items-center space-x-1.5 px-3 py-1.5 rounded-full bg-cyan-500 text-slate-950 text-xs font-bold shadow-lg">
