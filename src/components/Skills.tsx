...
export const Skills: React.FC = () => {
  ...
  return (
    <section id="skills" className="py-24 px-4 sm:px-6 relative tech-grid bg-slate-50/40">
          ...
          {skillsData.map((category, index) => {
            ...
            return (
              <motion.div
                ...
                className="p-6 sm:p-7 rounded-2xl bg-white border border-slate-200 hover:border-slate-300 transition-all duration-300 shadow-xs relative overflow-hidden"
              >
                {/* Header of card */}
                <div className="flex items-start space-x-3.5 mb-4">
                  <div className="p-3 rounded-xl bg-cyan-50 border border-cyan-100 text-cyan-700 shrink-0">
                  ...
                  {category.skills.map((skill) => (
                    <div
                      ...
                      className={`px-3 py-1.5 rounded-xl text-xs font-mono flex items-center space-x-1.5 transition-colors ${
                        skill.highlight
                          ? 'bg-cyan-50 text-cyan-800 border border-cyan-300/80 shadow-2xs font-semibold'
                    ...
                    >
                      ...
                      <span className="text-[10px] text-slate-400 font-sans pl-0.5">
                        • {skill.level}
                      </span>
                    </div>
                  ))}
